...
class FoldCompleteAnimator extends StatefulWidget {
  ...
  final int gridRows;
  final int gridCols;
  /// True when this puzzle was already completed before this solve —
  /// no XP was actually awarded, so the XP stat/star is omitted entirely.
  final bool hideXP;
...
class FoldCompleteAnimatorState extends State<FoldCompleteAnimator> with TickerProviderStateMixin {
  // Stage 0: lift  |  Stage 1: fold in half  |  Stage 2: turn 90°  |  Stage 3: stamp  |  Stage 4: stats
  ...
  Future<void> _run() async {
    ...
    await _liftCtrl.forward();  setState(() => _stage = 1);
    await _foldCtrl.forward();  setState(() => _stage = 2);
    await _turnCtrl.forward();  setState(() => _stage = 3);
  ...
  Widget build(BuildContext context) {
    ...
    // Tighter stage — stats now sit close to the card instead of far below it.
            ...
            child: ClipRect(
              ...
              child: OverflowBox(
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (context, _) {
                    // ── Stage 0: lift — grey panel rises + fades in. No fold, no colour change.
                    if (_stage == 0) {
                    ...
                    // ── Stage 1: fold — grey book-fold in place. Still grey, still in place.
                    if (_stage == 1) {
                    ...
                    // ── Stage 2/3: no flying anywhere — the folded panel just spins
                    // 90° in place (around its own centre) while it morphs into the
                    // final landscape card size and darkens to its solved colour.
                    final tt = _turnCtrl.value.clamp(0.0, 1.0);
                    final eased = Curves.easeInOutCubic.transform(tt);
                    final angle = eased * (math.pi / 2); // quarter turn, once
                    ...
                    return Transform.translate(
                      ...
                      child: Transform(
                        ...
                        transform: Matrix4.identity()
                          ..setEntry(3, 2, 0.0009)
                          ..rotateZ(angle * 0.001) // negligible — kept for a subtle wobble
                          ..rotateY(angle),
                        child: Container(
                          width: w, height: h,
